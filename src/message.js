...
async function GroupUpdate(seth, update, store) {
	try {
		for (let n of update) {
...
async function GroupParticipantsUpdate(seth, { id, participants, action }, store) {
	try {
		if (global.db.groups && global.db.groups[id] && global.db.groups[id].welcome && store.groupMetadata && store.groupMetadata[id]) {
			...
			for (let n of participants) {
				let profile;
				try {
					profile = await seth.profilePictureUrl(n, 'image');
				} catch {
					profile = 'https://telegra.ph/file/95670d63378f7f4210f03.png';
				}
				if (action == 'add') {
					await seth.sendMessage(id, {
...
async function LoadDataBase(seth, m) {
	try {
...
async function MessagesUpsert(seth, message, store) {
	try {
		let botNumber = await seth.decodeJid(seth.user.id);
		const msg = message.messages[0];
		if (store.groupMetadata && Object.keys(store.groupMetadata).length === 0) store.groupMetadata = await seth.groupFetchAllParticipating()
		const type = msg.message ? (getContentType(msg.message) || Object.keys(msg.message)[0]) : '';
		if (!seth.public && !msg.key.fromMe && message.type === 'notify') return
		if (msg.key.id.startsWith('BAE5')) return
		if (msg.key.id.length === 22) return
		if (!msg.message) return
		const m = await Serialize(seth, msg, store)
		require('../seth')(seth, m, message, store);
		if (type === 'interactiveResponseMessage' && m.quoted && m.quoted.fromMe) {
			let apb = await generateWAMessage(m.chat, { text: JSON.parse(m.msg.nativeFlowResponseMessage.paramsJson).id, mentions: m.mentionedJid }, {
				userJid: seth.user.id,
				quoted: m.quoted
			});
			apb.key = msg.key
			apb.key.fromMe = areJidsSameUser(m.sender, seth.user.id);
			...
			seth.ev.emit('messages.upsert', pbr);
		}
		if (global.db.set && global.db.set[botNumber] && global.db.set[botNumber].readsw) {
			if (msg.key.remoteJid === 'status@broadcast') {
				await seth.readMessages([msg.key]);
				if (/protocolMessage/i.test(type)) seth.sendFromOwner(global.owner, 'Status dari @' + msg.key.participant.split('@')[0] + ' Telah dihapus', msg, { mentions: [msg.key.participant] });
				if (/(audioMessage|imageMessage|videoMessage|extendedTextMessage)/i.test(type)) {
					...
					await seth.sendFromOwner(global.owner, `Melihat story dari @${msg.key.participant.split('@')[0]}\n${keke}`, msg, { mentions: [msg.key.participant] });
				}
			}
		}
	} catch (e) {
...
async function Solving(seth, store) {
	seth.public = true
	
	seth.serializeM = (m) => MessagesUpsert(seth, m, store)
	...
	seth.getName = (jid, withoutContact  = false) => {
		const id = seth.decodeJid(jid);
		if (id.endsWith('@g.us')) {
			const groupInfo = store.contacts[id] || seth.groupMetadata(id) || {};
	...
	seth.sendContact = async (jid, kon, quoted = '', opts = {}) => {
		let list = []
		for (let i of kon) {
			list.push({
				displayName: await seth.getName(i + '@s.whatsapp.net'),
				vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await seth.getName(i + '@s.whatsapp.net')}\nFN:${await seth.getName(i + '@s.whatsapp.net')}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Ponsel\nitem2.ADR:;;Indonesia;;;;\nitem2.X-ABLabel:Region\nEND:VCARD` //vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await seth.getName(i + '@s.whatsapp.net')}\nFN:${await seth.getName(i + '@s.whatsapp.net')}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Ponsel\nitem2.EMAIL;type=INTERNET:whatsapp@gmail.com\nitem2.X-ABLabel:Email\nitem3.URL:https://instagram.com/seth_dev\nitem3.X-ABLabel:Instagram\nitem4.ADR:;;Indonesia;;;;\nitem4.X-ABLabel:Region\nEND:VCARD`
			})
		}
		seth.sendMessage(jid, { contacts: { displayName: `${list.length} Kontak`, contacts: list }, ...opts }, { quoted })
	}
	
	seth.profilePictureUrl = async (jid, type = 'image', timeoutMs) => {
		const result = await seth.query({
			tag: 'iq',
			attrs: {
	...
	seth.sendPoll = (jid, name = '', values = [], selectableCount = 1) => {
		return seth.sendMessage(jid, { poll: { name, values, selectableCount }})
	}
	
	seth.sendFileUrl = async (jid, url, caption, quoted, options = {}) => {
		async function getFileUrl(res, mime) {
			if (mime.includes('gif')) {
				return seth.sendMessage(jid, { video: res.data, caption: caption, gifPlayback: true, ...options }, { quoted });
			} else if (mime === 'application/pdf') {
				return seth.sendMessage(jid, { document: res.data, mimetype: 'application/pdf', caption: caption, ...options }, { quoted });
			} else if (mime.includes('image')) {
				return seth.sendMessage(jid, { image: res.data, caption: caption, ...options }, { quoted });
			} else if (mime.includes('video')) {
				return seth.sendMessage(jid, { video: res.data, caption: caption, mimetype: 'video/mp4', ...options }, { quoted });
			} else if (mime.includes('audio')) {
				return seth.sendMessage(jid, { audio: res.data, mimetype: 'audio/mpeg', ...options }, { quoted });
			}
		}
	...
	seth.sendFakeLink = async (jid, text, title, body, thumbnail, myweb, options = {}) => {
		await seth.sendMessage(jid, {
			text: text,
			contextInfo: {
				externalAdReply: {
					title: title,
	...
	seth.sendFromOwner = async (jid, text, quoted, options = {}) => {
		for (const a of jid) {
			await seth.sendMessage(a.replace(/[^0-9]/g, '') + '@s.whatsapp.net', { text, ...options }, { quoted });
		}
	}
	
	seth.sendTextMentions = async (jid, text, quoted, options = {}) => seth.sendMessage(jid, { text: text, mentions: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net'), ...options }, { quoted })
	
	seth.sendAsSticker = async (jid, path, quoted, options = {}) => {
		...
		await seth.sendMessage(jid, { sticker: { url: result }, ...options }, { quoted });
		return buff;
	}
	
	seth.downloadAndSaveMediaMessage = async (message, filename, attachExtension = true) => {
	...
	seth.sendMedia = async (jid, path, fileName = '', caption = '', quoted = '', options = {}) => {
		const { mime, data, filename } = await seth.getFile(path, true);
		...
		await seth.sendMessage(jid, { [type]: { url: pathFile }, caption, mimetype, fileName, ...options }, { quoted, ...options });
		return fs.promises.unlink(pathFile);
	}
	
	seth.sendButtonMsg = async (jid, body = '', footer = '', title = '', media, buttons = [], quoted, options = {}) => {
		...
		await seth.relayMessage(msg.key.remoteJid, msg.message, { messageId: msg.key.id });
	}
	
	seth.sendCarouselMsg = async (jid, body = '', footer = '', cards = [], options = {}) => {
		async function getImageMsg(url) {
			const { imageMessage } = await generateWAMessageContent({ image: { url } }, { upload: seth.waUploadToServer });
			return imageMessage;
		}
...
async function Serialize(seth, m, store) {
	const botNumber = seth.decodeJid(seth.user.id)
	if (!m) return m
	if (m.key) {
		m.id = m.key.id
		...
		m.sender = seth.decodeJid(m.fromMe && seth.user.id || m.participant || m.key.participant || m.chat || '')
		if (m.isGroup) {
			m.metadata = store.groupMetadata[m.chat] || await seth.groupMetadata(m.chat)
	...
	if (m.message) {
		...
		if (m.quoted) {
			...
			m.quoted.sender = seth.decodeJid(m.msg.contextInfo.participant)
			m.quoted.fromMe = m.quoted.sender === seth.decodeJid(seth.user.id)
			...
			m.getQuotedObj = async () => {
				...
				let q = await store.loadMessage(m.chat, m.quoted.id, seth)
				return await Serialize(seth, q, store)
			}
			m.quoted.key = {
				remoteJid: m.msg?.contextInfo?.remoteJid || m.chat,
				participant: m.quoted.sender,
				fromMe: areJidsSameUser(seth.decodeJid(m.msg?.contextInfo?.participant), seth.decodeJid(seth?.user?.id)),
			...
			m.quoted.delete = () => {
				seth.sendMessage(m.quoted.chat, {
					delete: {
						remoteJid: m.quoted.chat,
	...
	m.copy = () => Serialize(seth, proto.WebMessageInfo.fromObject(proto.WebMessageInfo.toObject(m)))
		...
		try {
			if (/^https?:\/\//.test(text)) {
				...
				if (/gif|image|video|audio|pdf/i.test(mime)) {
					return seth.sendFileUrl(chatId, text, caption, quoted, options)
				} else {
					return seth.sendMessage(chatId, { text: text, mentions: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net'), ...options }, { quoted })
				}
			} else {
				return seth.sendMessage(chatId, { text: text, mentions: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net'), ...options }, { quoted })
			}
